...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="argusujin/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/argusujin/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/argusujin/guestbook-config.git'
                }
            }
        }
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_argusujin') {
                        oDockImage.push()
                    }
                }
            }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://argusujin:${GITHUB_ACCESS_TOKEN}@github.com/argusujin/guestbook-config.git"
                    '''
                }
            }
        }
